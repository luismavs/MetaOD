...
'''
    ...
    data_headers = perf_mat[2:, 0]
    config_headers = perf_df.columns[4:]
    dump(config_headers, 'model_list.joblib')

    # model_list must be the same as config_headers
    # n_datasets must be 2    
    
    # build meta-features -----------------------------------------------
    meta_mat = np.zeros([n_datasets, len(model_list)])
    
    for dd in data_list:
        # function to generate meta_mat from a model_name/path
        mat = loadmat(_path_to_mat_file_)
        X = mat['X']
        meta_mat[dd, :], meta_vec_names = generate_meta_features(X)
